...
struct L10n {
    ...
    var hours: String { lang == .zh ? "小时" : "hrs" }
    var repeatForeground: String { lang == .zh ? "连续前置" : "Repeat count" }
    var times: String { lang == .zh ? "次" : "x" }
    ...
    var helpSmartUnlock: String {
        lang == .zh
            ? "仅在锁屏时长超过设定阈值时才触发。\n解锁后会连续前置多次，确保目标应用保持在最前面。\n适合午休、离开等长时间锁屏后使用。"
            : "Only triggers when lock duration exceeds threshold.\nForegrounds repeatedly after unlock to keep app in focus.\nIdeal after lunch breaks or extended absences."
    }
    ...
    var helpThreshold: String {
        lang == .zh ? "锁屏超过此时长才会触发前置" : "Foreground only triggers after this lock duration"
    }
    var helpRepeat: String {
        lang == .zh ? "解锁后连续前置的次数，每次间隔2秒" : "Number of foreground attempts after unlock, 2s apart"
    }
}
